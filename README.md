...
Exercise ratings and history are stored per-device in `localStorage`. Shared plans, club data, and account authorization go through [`sports-training-api`](https://github.com/BojanKocijan/sports-training-api) (`src/lib/apiClient.ts`) — the browser never uses the Supabase service-role key. Trainers and parents use named Supabase Auth accounts. A trainer links a parent's email to a child; the invite link signs that parent in directly and the API grants read-only access only to linked children. Email OTP remains a login fallback. Set `VITE_API_URL` (see `.env.example`) to point at your API deployment. The API needs the matching account migration, Auth email templates, invitation redirect, and SMTP configuration; see its README.
