...
/** Screenshots the 3D mascot preview inside the real create-player wizard (not the hidden
 * /poc-3d.html dev page), for each mascot that has a 3D model — so a layout/camera/framing
 * regression has a picture, not just a description, to attach to its issue. Saved under
 * test-results/ (gitignored) via `testInfo.outputPath` so `npx playwright test mascot-3d` prints
 * exactly where to find it. Not a golden-image diff (three.js output is not pixel-stable across
 * GPUs/drivers), so this only asserts the canvas rendered — read the screenshot yourself. */
test.describe('3D mascot preview', () => {
  test.beforeEach(async ({ page }) => {
    await mockGate(page)
    await page.route(`${API_URL}/players?groupId=${GROUP_ID}`, (route) => route.fulfill({ json: [] }))
  ...
  for (const mascot of ['lion', 'shark'] as const) {
    test(`${mascot}: 3D toggle renders and can be screenshotted`, async ({ page }, testInfo) => {
      ...
      await page.getByRole('button', { name: 'Next' }).click() // nickname -> animal

      if (mascot !== 'lion') {
        await page.getByRole('button', { name: new RegExp(mascot, 'i') }).click()
      }
